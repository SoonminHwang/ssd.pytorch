...
class RefineDet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB, num_classes):
        ...
        if phase == 'test':
            ...
            # self.detect = Detect_RefineDet(num_classes, self.size, 0, 1000, 0.1, 0.45, 0.01, 500)
...
def vgg(cfg, i, batch_norm=False):
    ...
    pool5 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0, ceil_mode=True)
    conv6 = nn.Conv2d(512, 1024, kernel_size=3, padding=3, dilation=3)

    # ## original ssd-stype params for pool5, conv6
...
def build_refinedet(phase, size=320, num_classes=21):
    ...
    if size != 300 and size != 320 and size != 512:
        print("ERROR: You specified size " + repr(size) + ". However, " +
              "currently only RefineDet300 and RefineDet320 and RefineDet512 is supported!")
        return
    ...
    ### for size == 300,    
    TCB_[1][1].output_padding = (1,1)
    TCB_[1][1].padding = (1,1)
