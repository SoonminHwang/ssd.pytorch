...
from data import voc_stairnet300_38x38 as voc_stairnet300
...
def multibox(vgg, extra_layers, cfg, num_classes, inter_planes):
    ...
    for k, v in enumerate(vgg_source):
        ...
        conf_layers += [nn.Conv2d(inter_planes,
                        cfg[k] * num_classes, kernel_size=3, padding=1)]
    # for k, v in enumerate(extra_layers[1::2], 2):
    for k, v in enumerate(extra_layers[1:-1:2], 2):
...
mbox = {
    '300': [6, 6, 6, 6, 4],  # number of boxes per feature map location
    '512': [],
}
