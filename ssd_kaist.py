...
class SSD(nn.Module):
    ...
    def __init__(self, phase, lwir, base, extras, head, num_classes):
        ...
        vgg16_weights = nn.ModuleList( [*vgg16.features.children()] )
        vgg16_state = vgg16_weights.state_dict()
        vgg16_state_keys = [ *vgg16_state.keys() ]
        

        loaded_state = OrderedDict()
        state = self.vgg.state_dict()
        state_keys = state.keys()
        
        for k in state_keys:
            if k in vgg16_state_keys:
                loaded_state[k] = vgg16_state[k]
        ...
        loaded_state = OrderedDict()
        state = self.lwir.state_dict()
        state_keys = state.keys()

        vgg16_state['0.weight'] = torch.mean( vgg16_state['0.weight'], dim=1, keepdim=True )
        for k in state_keys:
            if k in vgg16_state_keys:
                loaded_state[k] = vgg16_state[k]

        state.update(loaded_state)    
        self.lwir.load_state_dict(state)
