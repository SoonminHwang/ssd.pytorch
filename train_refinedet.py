...
def train():
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        if (iteration+1) % 10 == 0:
            ...
            logger.info('iter ' + repr(iteration) + ' || Loss: %.4f || ARM_L Loss: %.4f ARM_C Loss: %.4f ODM_L Loss: %.4f ODM_C Loss: %.4f ||' \
                % (acc_loss/10.0, acc_arm_loc_loss/10.0, acc_arm_conf_loss/10.0, acc_odm_loc_loss/10.0, acc_odm_conf_loss/10.0))
